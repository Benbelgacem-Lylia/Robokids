...
const StudentTableComponent = ({ data }) => {
  ...
  const handleEdit = (id) => {
    navigate(`/controlpannel/editstudent/${id}`); // Navigate to the edit page
  };
      ...
      <Table sx={{ minWidth: 650 }} aria-label="custom table">
        <TableHead>
          <TableRow>
            ...
            <StyledTableCell>{t('Courses')}</StyledTableCell>
        ...
        <TableBody>
          {data.map((row) => (
            <TableRow key={row.id}>
              ...
              <TableCell>{row.studyProgram}</TableCell>
